...
void	main(void)	{	
    ...
    fColor = (1-noShadow)*(Ie + Ia) + noShadow*(Ie + Ia + Id + Is) + fColor; 
     
    if (dSphere==1) fColor = 0.8*fColor + 0.2*texture(cubeTex, tc3); // integrate color and texture
    else if (dCone==1) fColor = 0.8*fColor + 0.2*texture(vseTex, tc2); // integrate color and texture
    else fColor = 0.8*fColor + 0.2*texture(gmuTex, tc1); // integrate color and texture
