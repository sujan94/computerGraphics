...
public class H1_4_font_sprajap2 extends JOGL1_1_PointVFfiles {

    private static final float RADIUS = 0.5f;
    private static final int CIRCLE_POINTS_COUNT = 300;
    private static final int BOUNCE_POINTS_COUNT = 30;
    private static final double ANGLE_INCREMENT = 2 * Math.PI / CIRCLE_POINTS_COUNT;
    private static final float PENTAGON_POINT_MOVE_THETA_VALUE = 0.07f;
    List<Float> pointSpeedList = new ArrayList<>();
    ...
    private final float[] bouncingPointList;
    ...
    public H1_4_font_sprajap2() {
        ...
        bouncingPointList = generateBouncingPoints(BOUNCE_POINTS_COUNT);
        generatePointBoundBoolean();
        nameFirstInitials = generateInitialPoints('s');
        nameLastInitials = generateInitialPoints('p');
    }

    private void generatePointBoundBoolean() {
        pointSpeedList.clear();
        Random random = new Random();
        for (float f : bouncingPointList) {
            pointSpeedList.add((random.nextBoolean()? 1: -1)*(random.nextFloat() /10));
        }
    }

    private float[] generateRandomColor(int size) {
        float[] color = new float[size];
        for (int i = 0; i < color.length; i++) {
            color[i] = new Random().nextFloat();
        }
        return color;
    }

    private float[] generateBouncingPoints(int points) {
        float[] list = new float[3 * points];
    ...
    private float[] generateCirclePoints() {
        ...
        float[] circlePoints = new float[3 * CIRCLE_POINTS_COUNT]; // predefined number of pixels on the line
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        drawWithAnimation(nameFirstInitials, generateRandomColor(nameFirstInitials.length));
        myTranslatef(0.8f, 0, 0);
        drawWithAnimation(nameLastInitials, generateRandomColor(nameFirstInitials.length));
        myLoadIdentity();

        // Draw Initial inside the circle
        myLoadIdentity();
        ...
        // Draw bouncing points
        myLoadIdentity();
        animateBouncingPoint(bouncingPointList, generateRandomColor(3));
        myLoadIdentity();
    ...
    public void animateBouncingPoint(float[] bouncingPointList, float[] color) {
        gl.glPointSize(6.0f);

        // Calculate the ball's new position
        int i = 0;
        while (i < bouncingPointList.length) {
            float pointx = bouncingPointList[i];
            float pointy = bouncingPointList[i + 1];
            float speedX = pointSpeedList.get(i);
            float speedY = pointSpeedList.get(i+1);
            float currentx= pointx;
            float currenty = pointy;
            currentx += speedX;
            currenty += speedY;
            float currentDistance = (float) Math.abs(Math.hypot(currentx, currenty));
            if (currentx  < 0 && currentDistance>RADIUS){
                speedX= Math.abs(speedX);
                currentx = currentx+speedX;
            }
            else if (currentx  > 0 && currentDistance >RADIUS) {
                currentx = currentx-speedX;
                speedX = -speedX;
            }

            // May cross both x and y bounds
            if (currenty  < 0 && currentDistance> RADIUS) {
                speedY = Math.abs(speedY);
                currenty = currenty+speedY;
            } else if (currenty  > 0 && currentDistance > RADIUS) {
                currenty = currenty-speedY;
                speedY = -speedY;
            }

            System.out.println("x= "+currentx+", y="+currenty+" speedX "+speedX+" speedY="+ speedY);

            pointSpeedList.add(i, speedX);
            pointSpeedList.add(i+1, speedY);

            if(Math.abs(Math.hypot(currentx, currenty))>RADIUS){
                currentx = pointx;
                currenty = pointy;
            }

            bouncingPointList[i] = currentx;
            bouncingPointList[i+1] = currenty;
            i = i + 3;
            drawPointArray(new float[]{currentx, currenty, 0.0f},generateRandomColor(3), GL_POINTS);
    ...
    public void drawWithAnimation(float[] v1, float[] randomColorList) {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        vShaderSource = readShaderSource("src/project2/H1_4_font_sprajap2_V.shader"); // read vertex shader
        fShaderSource = readShaderSource("src/project2/H1_4_font_sprajap2_F.shader"); // read fragment shader
    ...
    public static void main(String[] args) {
        H1_4_font_sprajap2 project = new H1_4_font_sprajap2();
